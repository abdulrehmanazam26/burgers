  ...
  useLayoutEffect(()=>{
    ...
    mm.add({desktop:"(min-width:700px) and (prefers-reduced-motion:no-preference)",mobile:"(max-width:699px) and (prefers-reduced-motion:no-preference)"},ctx=>{
      ...
      tl.addLabel("complete").to(".label-intro",{opacity:0,duration:.2},"<").to(".complete-copy",{opacity:1,y:0,duration:.45}).to(".complete-copy",{opacity:0,y:-24,duration:.3},">+.45")
        .addLabel("package").to(".package",{opacity:1,y:0,scale:1,rotateX:48,duration:.7,ease:"power3.inOut"}).to(".burger-stage",{y:mobile?110:138,scale:mobile?.46:.52,duration:1.05,ease:"power3.inOut"}).to(".burger-stage",{scale:mobile?.44:.5,duration:.3,ease:"power2.out"}).to(".burger-stage",{opacity:0,duration:.16},">+.25").to(".box-lid",{rotateX:0,duration:.78,ease:"power3.inOut"}).to(".box-interior",{opacity:0,duration:.15},"<+.45").to(".package",{rotateX:0,y:-15,scale:.92,duration:.85,ease:"power4.inOut"}).to(".hero-outro",{opacity:1,duration:.35},">+.15");
    });
    return()=>mm.revert();
  },[]);
